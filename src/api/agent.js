...
//TODO: convert to TS and add types for propertyDetails to ensure it has address field.
const realEstateAgent = async ({ propertyDetails }) => {
  if (!propertyDetails?.address) return null;
  ...
  const agentFinalState = await agent.invoke(
    {
      messages: [
        new HumanMessage(
          `what is the walk score of ${propertyDetails?.address}?`
        ),
      ],
    },
  ...
  const agentNextState = await agent.invoke(
    {
      messages: [
        new HumanMessage(
          "how many restaurants are close to this property within a 0.5 mile radius and which cuisines?"
        ),
      ],
    },
    { configurable: { thread_id: "42" } }
  ...
  console.log(
    agentOpenRestaurantState.messages[
      agentOpenRestaurantState.messages.length - 1
    ].content
  );
